...
비동기 컴포넌트를 앞 장에서 만들어준 [AsyncBoundary](https://jbee.io/articles/react/error-declarative-handling-0/)로 감싸주는 경우, 에러가 **전역으로 퍼지는 것을 방지**할 수 있다. 에러가 발생했을 경우, ErrorBoundary에서 catch되기 때문이다. 컴포넌트에서 바로 사용할 수 있도록 일반적인 pendingFallback과 rejectedFallback을 전달한 AsyncBoundary를 만들어두고 사용하자.
